/**
 * server.js — Dashboard Diretoria C — Cupons VTEX
...
app.get('/api/vtex-debug', async (req, res) => {
  ...
  try {
    ...
    const matchedCintia = withCoupon.filter(o => {
    ...
    const couponSellers = new Set();
    const matchedSellers = new Set();
    orderValues.forEach(o => {
      const seller = o.sellers?.[0]?.name || '';
      if (seller) allSellers.add(seller);
    });
    withCoupon.forEach(o => {
      const seller = o.sellers?.[0]?.name || '';
      if (seller) couponSellers.add(seller);
    });
    matchedCintia.forEach(o => {
      const seller = o.sellers?.[0]?.name || '';
      const cleanSeller = seller.includes(' - ') ? seller.split(' - ')[0].trim() : seller;
      const info = lookupStore(cleanSeller);
      if (info) matchedSellers.add(info.matchedKey);
    });
    ...
    res.json({
      ...
      orders_matched_cintia: matchedCintia.length,
      unique_sellers_count:  allSellers.size,
      matched_sellers_count: matchedSellers.size,
      sellers_sample:        [...allSellers].slice(0, 10),
      coupon_sellers_sample: [...couponSellers].slice(0, 10),
      matched_sellers_sample:[...matchedSellers].slice(0, 10),
...
app.listen(PORT, () => {
  console.log(`\n🚀 Dashboard Diretoria C — Cupons VTEX — http://localhost:${PORT}`);
