...
<body>
      ...
      <div class="card" style="margin-top: 16px;">
        <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px;">
          ...
          <div class="btn-group-export">
            <button class="btn btn-sm btn-excel" onclick="exportDistritaisOverview('excel')" title="Baixar resumo das distritais em Excel (.xlsx)">📊 Baixar Excel</button>
            <button class="btn btn-sm" onclick="exportDistritaisOverview('csv')" title="Baixar resumo das distritais em CSV">📄 CSV</button>
          </div>
        </div>
        <div class="table-container">
          <table id="tableDistritaisOverview">
            <thead>
              <tr>
    ...
    <section id="tab-distritais" class="tab-content hidden">
      <div class="card">
        <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px;">
          ...
          <div class="btn-group-export">
            <button class="btn btn-sm btn-excel" onclick="exportDistritaisFull('excel')" title="Baixar ranking das distritais em Excel (.xlsx)">📊 Baixar Excel</button>
            <button class="btn btn-sm" onclick="exportDistritaisFull('csv')" title="Baixar ranking das distritais em CSV">📄 CSV</button>
          </div>
        </div>
        <div class="table-container">
    ...
    <section id="tab-coordenadores" class="tab-content hidden">
      <div class="card">
        <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px;">
          ...
          <div class="btn-group-export">
            <button class="btn btn-sm btn-excel" onclick="exportCoordenadoresFull('excel')" title="Baixar ranking dos coordenadores em Excel (.xlsx)">📊 Baixar Excel</button>
            <button class="btn btn-sm" onclick="exportCoordenadoresFull('csv')" title="Baixar ranking dos coordenadores em CSV">📄 CSV</button>
          </div>
        </div>
        <div class="table-container">
      ...
      <div class="card">
        <div class="card-header">
          ...
          <div style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">
            ...
            <div class="btn-group-export">
              <button class="btn btn-sm btn-excel" onclick="exportFiliaisFull('excel')" title="Baixar todas as filiais filtradas em Excel (.xlsx)">📊 Baixar Excel</button>
              <button class="btn btn-sm" onclick="exportFiliaisFull('csv')" title="Baixar todas as filiais filtradas em CSV">📄 CSV</button>
            </div>
          </div>
        </div>
        <div class="table-container">
      ...
      <div class="card">
        <div class="card-header" style="flex-wrap: wrap; gap: 10px;">
          ...
          <div style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">
            ...
            <div class="btn-group-export">
              <button class="btn btn-sm btn-excel" onclick="exportCategoriasFull('excel')" title="Baixar categorias e linhas completas em Excel (.xlsx)">📊 Baixar Excel</button>
              <button class="btn btn-sm" onclick="exportCategoriasFull('csv')" title="Baixar categorias em CSV">📄 CSV</button>
            </div>
          </div>
        </div>
        <div class="table-container">
      ...
      <div class="card">
        <div class="card-header" style="flex-wrap: wrap; gap: 12px;">
          ...
          <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
            ...
            <div class="btn-group-export">
              <button class="btn btn-sm btn-excel" onclick="exportLinhasFull('excel')" title="Baixar todas as linhas de produtos filtradas em Excel (.xlsx)">📊 Baixar Excel</button>
              <button class="btn btn-sm" onclick="exportLinhasFull('csv')" title="Baixar todas as linhas de produtos filtradas em CSV">📄 CSV</button>
            </div>
          </div>
        </div>
        <div class="table-container">
  ...
  <script>
    ...
    function getActiveTargetItem() {{
      ...
      // 1. COMBINAÇÃO COORDENADOR + LINHA
      if (selCoord !== 'all' && selLinha !== 'all') {{
        const foundCoord = (DASH_DATA.coordenadores || []).find(c => c.nome === selCoord);
        if (foundCoord && foundCoord.grupos) {{
          for (const g of foundCoord.grupos) {{
            const l = (g.linhas || []).find(x => x.linha === selLinha);
            if (l) {{
              const itemLinha = {{ ...l, _parentGrp: g }};
              return {{ 
                item: itemLinha, 
                type: 'Linha', 
                name: `${{l.linha}} (${{foundCoord.nome}})`, 
                parent: `Coord: ${{foundCoord.nome}} • ${{g.grupo}}`, 
                lojas: foundCoord.lojas,
                distrital: foundCoord.distrital,
                coordenador: foundCoord.nome,
                grupo: g.grupo
              }};
            }}
          }}
      ...
      // 2. COMBINAÇÃO DISTRITAL + LINHA
      if (selDist !== 'all' && selLinha !== 'all') {{
        const foundDist = (DASH_DATA.distritais || []).find(d => d.nome === selDist);
        if (foundDist && foundDist.grupos) {{
          for (const g of foundDist.grupos) {{
            const l = (g.linhas || []).find(x => x.linha === selLinha);
            if (l) {{
              const itemLinha = {{ ...l, _parentGrp: g }};
              return {{ 
                item: itemLinha, 
                type: 'Linha', 
                name: `${{l.linha}} (${{foundDist.nome}})`, 
                parent: `Distrital: ${{foundDist.nome}} • ${{g.grupo}}`, 
                lojas: foundDist.lojas,
                distrital: foundDist.nome,
                grupo: g.grupo
              }};
            }}
          }}
      ...
      // 3. COMBINAÇÃO COORDENADOR + GRUPO
      if (selCoord !== 'all' && selGrupo !== 'all') {{
        const foundCoord = (DASH_DATA.coordenadores || []).find(c => c.nome === selCoord);
        if (foundCoord && foundCoord.grupos) {{
          const g = foundCoord.grupos.find(x => x.grupo === selGrupo);
          if (g) {{
            return {{
              item: g,
              type: 'Grupo',
              name: `${{g.grupo}} (${{foundCoord.nome}})`,
              parent: `Coord: ${{foundCoord.nome}} (${{foundCoord.distrital}})`,
              lojas: foundCoord.lojas,
              distrital: foundCoord.distrital,
              coordenador: foundCoord.nome,
              grupo: g.grupo
      ...
      // 4. COMBINAÇÃO DISTRITAL + GRUPO
      if (selDist !== 'all' && selGrupo !== 'all') {{
        const foundDist = (DASH_DATA.distritais || []).find(d => d.nome === selDist);
        if (foundDist && foundDist.grupos) {{
          const g = foundDist.grupos.find(x => x.grupo === selGrupo);
          if (g) {{
            return {{
              item: g,
              type: 'Grupo',
              name: `${{g.grupo}} (${{foundDist.nome}})`,
              parent: `Distrital: ${{foundDist.nome}}`,
              lojas: foundDist.lojas,
              distrital: foundDist.nome,
              grupo: g.grupo
            }};
      ...
      // 5. PRIORIDADE DO ÚLTIMO FILTRO TOCADO
      ...
      // 6. FALLBACKS SEQUENCIAIS SE lastFilterType NÃO COUBER
      if (selLinha !== 'all') {{
        const found = (DASH_DATA.linhas || []).find(l => l.linha === selLinha);
        if (found) return {{ item: found, type: 'Linha', name: found.linha, parent: found.grupo, lojas: null }};
      }}
    ...
    function updateKpiFilterContext(activeTarget, search) {{
      ...
      const isFiltered = (activeTarget.type !== 'Diretoria') || (search && search.length > 0) || (selDist !== 'all') || (selCoord !== 'all') || (selGrupo !== 'all') || (selLinha !== 'all');
      ...
      if (banner) {{
        if (isFiltered) {{
          ...
          let parts = [];
          if (selDist !== 'all') parts.push(`Distrital: ${{selDist}}`);
          if (selCoord !== 'all') parts.push(`Coord: ${{selCoord}}`);
          if (selGrupo !== 'all') parts.push(`Grupo: ${{selGrupo}}`);
          if (selLinha !== 'all') parts.push(`Linha: ${{selLinha}}`);
          if (search) parts.push(`Busca: "${{search}}"`);

          if (tagEl) {{
            if (parts.length > 1) tagEl.textContent = 'Filtro Combinado';
            else tagEl.textContent = activeTarget.type !== 'Diretoria' ? activeTarget.type : 'Busca';
          }}
          if (textEl) {{
            textEl.textContent = parts.length > 0 ? parts.join(' • ') : activeTarget.name;
          }}
          if (detailsEl) {{
            let det = [];
            if (activeTarget.lojas) det.push(`${{activeTarget.lojas}} Lojas Ativas`);
            if (activeTarget.item && activeTarget.item.total_linhas) det.push(`${{activeTarget.item.total_linhas}} Linhas`);
    ...
    function renderAllTables(filterDist = 'all', filterCoord = 'all', filterGrupoVal = 'all', filterLinhaVal = 'all', search = '') {{
        ...
        .map((d, origIdx) => {{
          ...
          if (filterLinhaVal !== 'all') {{
            for (const g of (d.grupos || [])) {{
              const l = (g.linhas || []).find(x => x.linha === filterLinhaVal);
              if (l) {{ itemToEval = {{ ...l, _parentGrp: g, nome: d.nome, lojas: d.lojas }}; break; }}
            }}
          }} else if (filterGrupoVal !== 'all') {{
            const g = (d.grupos || []).find(x => x.grupo === filterGrupoVal);
            if (g) itemToEval = {{ ...g, nome: d.nome, lojas: d.lojas }};
          }}
          const m = getPeriodMetrics(itemToEval);
          ...
          return {{
            ...
            nome: d.nome,
            lojas: d.lojas,
            meta_mes: itemToEval.meta_mes || d.meta_mes,
            ...m,
      ...
      window.currentDistritaisOverviewList = distOverviewList;
      ...
      window.currentDistritaisFullList = distFullList;
        ...
        .map((c, origIdx) => {{
          ...
          if (filterLinhaVal !== 'all') {{
            for (const g of (c.grupos || [])) {{
              const l = (g.linhas || []).find(x => x.linha === filterLinhaVal);
              if (l) {{ itemToEval = {{ ...l, _parentGrp: g, nome: c.nome, distrital: c.distrital, lojas: c.lojas }}; break; }}
            }}
          }} else if (filterGrupoVal !== 'all') {{
            const g = (c.grupos || []).find(x => x.grupo === filterGrupoVal);
            if (g) itemToEval = {{ ...g, nome: c.nome, distrital: c.distrital, lojas: c.lojas }};
          }}
          const m = getPeriodMetrics(itemToEval);
          const share_diretoria = dirTotDigital > 0 ? ((m.venda_digital / dirTotDigital) * 100) : 0.0;
          const diff_share = m.share_digital - dirMediaShare;
          ...
          return {{
            ...
            lojas: c.lojas,
            meta_mes: itemToEval.meta_mes || c.meta_mes,
            ...m,
            share_diretoria: share_diretoria,
            diff_share: diff_share,
            media_loja: media_loja
      ...
      window.currentCoordenadoresList = coordList;
      ...
      window.currentFiliaisList = filList;
      ...
      window.currentCategoriasList = catList;
    ...
    function renderLinhasTable(resetPage = false) {{
      ...
      if (coord !== 'all') {{
        const foundC = (DASH_DATA.coordenadores || []).find(c => c.nome === coord);
        if (foundC && foundC.grupos && foundC.grupos.length > 0) {{
          targetLinhas = foundC.grupos.flatMap(g => (g.linhas || []).map(l => ({{ ...l, _parentGrp: g }})));
          contextLinhasLabel = `Coordenador: ${{foundC.nome}} (${{foundC.distrital}})`;
        }}
      }} else if (dist !== 'all') {{
        const foundD = (DASH_DATA.distritais || []).find(d => d.nome === dist);
        if (foundD && foundD.grupos && foundD.grupos.length > 0) {{
          targetLinhas = foundD.grupos.flatMap(g => (g.linhas || []).map(l => ({{ ...l, _parentGrp: g }})));
          contextLinhasLabel = `Distrital: ${{foundD.nome}}`;
        }}
      }} else {{
        targetLinhas = DASH_DATA.linhas || [];
      }}
    ...
    function renderCharts(activeTarget) {{
      ...
      if (ctxCurva) {{
        ...
        let metasDiarias = target.metas_dias;
        let vDias = (activeFigitalMode === 'sem') 
          ? (target.vendas_dias_sem_figital || []) 
          : (target.vendas_dias_digital || []);

        if ((!metasDiarias || metasDiarias.length === 0) && target._parentGrp) {{
          const p = target._parentGrp;
          const ratioMeta = (p.meta_mtd > 0) ? ((target.meta_mtd || 0) / p.meta_mtd) : 1.0;
          const pBaseSale = (activeFigitalMode === 'sem') ? (p.venda_sem_figital || 1.0) : (p.venda_digital || 1.0);
          const tBaseSale = (activeFigitalMode === 'sem') ? (target.venda_sem_figital || 0.0) : (target.venda_digital || 0.0);
          const ratioSale = pBaseSale > 0 ? (tBaseSale / pBaseSale) : 1.0;
          metasDiarias = (p.metas_dias || []).map(m => Math.round(m * ratioMeta));
          const pVDias = (activeFigitalMode === 'sem') ? (p.vendas_dias_sem_figital || []) : (p.vendas_dias_digital || []);
          vDias = pVDias.map(v => Math.round(v * ratioSale));
        }}

        if (!metasDiarias || metasDiarias.length === 0) {{
          metasDiarias = (DASH_DATA.curva_diaria || []).map(d => d.meta_dia);
        }}
      ...
      if (ctxShare) {{
        ...
        if (targetType === 'Coordenador') {{
          if (titleEl) titleEl.textContent = `Participação das Filiais — ${{targetName}}`;
          if (subEl) subEl.textContent = 'Share de Venda Digital das Lojas do Coordenador';
          const filList = (DASH_DATA.filiais || []).filter(f => f.coordenador === targetName);
          const mList = filList.map(f => ({{ nome: f.nome, venda: getPeriodMetrics(f).venda_digital }})).sort((a,b) => b.venda - a.venda);
          const top7 = mList.slice(0, 7);
          const rest = mList.slice(7);
          const restSum = rest.reduce((acc, x) => acc + x.venda, 0);
          labels = top7.map(x => x.nome);
          values = top7.map(x => x.venda);
          if (restSum > 0) {{
            labels.push('Outras Filiais');
            values.push(restSum);
          }}
        }} else if (targetType === 'Distrital') {{
        ...
        }} else if (targetType === 'Linha') {{
          if (titleEl) titleEl.textContent = `Distribuição por Distrital — ${{targetName}}`;
          if (subEl) subEl.textContent = 'Share de Venda Digital da Linha entre as Distritais';
          const dList = DASH_DATA.distritais.map(d => {{
            let v = 0;
            for (const g of (d.grupos || [])) {{
              const l = (g.linhas || []).find(x => x.linha === target.linha);
              if (l) {{ v = getPeriodMetrics({{ ...l, _parentGrp: g }}).venda_digital; break; }}
            }}
            return {{ nome: d.nome, venda: v }};
          }}).sort((a,b) => b.venda - a.venda);
          labels = dList.map(x => x.nome);
          values = dList.map(x => x.venda);
        }} else {{
    ...
    // =========================================================================
    // EXPORTAÇÃO AVANÇADA (EXCEL NATIVO XLSX COM MULTI-SHEET & CSV FORMATADO)
    // =========================================================================

    function getStatusText(atingimento) {{
      if (atingimento >= 100) return 'Bateu Meta';
      if (atingimento >= 95) return 'Em Linha';
      return 'Abaixo';
    }}
    ...
    function exportSheets(sheetsDict, baseFilename, format = 'excel') {{
      const dateIni = document.getElementById('filterDateIni')?.value || '2026-09-01';
      const dateEnd = document.getElementById('filterDateEnd')?.value || '2026-09-30';
      const cleanFilename = `${{baseFilename}}_${{dateIni}}_a_${{dateEnd}}`.replace(/[^a-zA-Z0-9_\-]/g, '_');
      ...
      if (format === 'excel') {{
        if (typeof XLSX !== 'undefined' && XLSX.utils) {{
          ...
          for (const [sheetName, rows] of Object.entries(sheetsDict)) {{
            ...
            rows.forEach(r => {{
              r.forEach((c, idx) => {{
                const len = c != null ? String(c).length : 0;
          ...
          console.warn('SheetJS indisponível. Fazendo fallback para CSV UTF-8.');
          format = 'csv';
        }}
      }}

      if (format === 'csv') {{
        const firstKey = Object.keys(sheetsDict)[0];
        const rows = sheetsDict[firstKey] || [];
        let csvLines = [];
        for (let i = 0; i < rows.length; i++) {{
          const rowVals = rows[i].map(v => {{
            if (v == null) return '""';
            let str = String(v);
            if (typeof v === 'number') {{
              if (Number.isInteger(v)) str = String(v);
              else str = v.toFixed(2).replace('.', ',');
            }}
            str = str.replace(/"/g, '""');
            return `"${{str}}"`;
          }});
    ...
    function exportDistritaisOverview(format = 'excel') {{
      const list = window.currentDistritaisOverviewList || [];
      const headers = [
        'Distrital', 'Meta Período (R$)', 'Venda Digital (R$)', 'Progresso %',
        'GAP R$', 'Desvio %', 'Projeção Mês (R$)', 'Venda Total (R$)', 'Venda Física (R$)',
        'Share Atual %', 'Share LY %', 'Evol. P.P.', '% Share Diretoria', 'Lojas', 'R$ / Loja'
      ];
      const rows = [headers];
      list.forEach(d => {{
        rows.push([
          d.nome,
          Math.round(d.meta_periodo || 0),
          Math.round(d.venda_digital || 0),
          Number((d.atingimento || 0).toFixed(1)),
          Math.round(d.gap || 0),
          Number((d.desvio || 0).toFixed(1)),
          Math.round(d.projecao || 0),
          Math.round(d.venda_total || 0),
          Math.round(d.venda_fisica || 0),
          Number((d.share_digital || 0).toFixed(1)),
          Number((d.share_digital_ly || 0).toFixed(1)),
          Number((d.diff_pp_ly || 0).toFixed(1)),
          Number((d.share_diretoria || 0).toFixed(2)),
          d.lojas || 0,
          Math.round(d.media_loja || 0)
        ]);
      }});
      exportSheets({{ 'Resumo Distritais': rows }}, 'resumo_distritais_diretoria_c', format);
    }}

    function exportDistritaisFull(format = 'excel') {{
      const list = window.currentDistritaisFullList || [];
      const headers = [
        'Ranking', 'Distrital', 'Meta Mês (R$)', 'Meta Período (R$)', 'Venda Digital (R$)', 'Progresso %',
        'GAP R$', 'Desvio %', 'Projeção Mês (R$)', 'Venda Total (R$)', 'Venda Física (R$)',
        'Share Atual %', 'Share LY %', 'Evol. P.P.', '% Share Diretoria', 'Lojas', 'R$ / Loja', 'Status'
      ];
      const rows = [headers];
      list.forEach((d, idx) => {{
        rows.push([
          ...
          d.lojas || 0,
          Math.round(d.media_loja || 0),
          getStatusText(d.atingimento)
        ]);
      }});
      exportSheets({{ 'Ranking Distritais': rows }}, 'ranking_distritais_diretoria_c', format);
    }}

    function exportCoordenadoresFull(format = 'excel') {{
      const list = window.currentCoordenadoresList || [];
      const headers = [
        'Ranking', 'Coordenador', 'Distrital', 'Meta Mês (R$)', 'Meta Período (R$)', 'Venda Digital (R$)',
        'Atingimento %', 'GAP R$', 'Desvio %', 'Projeção Mês (R$)', 'Venda Total (R$)', 'Venda Física (R$)',
        'Share Atual %', 'Share LY %', 'Evol. P.P.', '% Share Diretoria', 'Lojas', 'R$ / Loja', 'Status'
      ];
      const rows = [headers];
      list.forEach((c, idx) => {{
        rows.push([
          ...
          c.nome,
          c.distrital,
          Math.round(c.meta_mes || 0),
          Math.round(c.meta_periodo || 0),
          Math.round(c.venda_digital || 0),
          Number((c.atingimento || 0).toFixed(1)),
          Math.round(c.gap || 0),
          Number((c.desvio || 0).toFixed(1)),
          Math.round(c.projecao || 0),
          Math.round(c.venda_total || 0),
          Math.round(c.venda_fisica || 0),
          Number((c.share_digital || 0).toFixed(1)),
          Number((c.share_digital_ly || 0).toFixed(1)),
          Number((c.diff_pp_ly || 0).toFixed(1)),
          Number((c.share_diretoria || 0).toFixed(2)),
          c.lojas || 0,
          Math.round(c.media_loja || 0),
          getStatusText(c.atingimento)
        ]);
      }});
      exportSheets({{ 'Ranking Coordenadores': rows }}, 'coordenadores_diretoria_c', format);
    }}

    function exportFiliaisFull(format = 'excel') {{
      const list = window.currentFiliaisList || [];
      const headers = [
        'Ranking', 'ID Loja', 'Filial / Loja', 'Distrital', 'Coordenador',
        'Meta Mês (R$)', 'Meta Período (R$)', 'Venda Digital (R$)', 'Atingimento %',
        'GAP R$', 'Desvio %', 'Projeção Mês (R$)', 'Venda Total (R$)', 'Venda Física (R$)',
        'Share Atual %', 'Share LY %', 'Evol. P.P.', '% Share Diretoria', 'Status'
      ];
      const rows = [headers];
      list.forEach((f, idx) => {{
        rows.push([
          ...
          f.distrital,
          f.coordenador,
          Math.round(f.meta_mes || 0),
          Math.round(f.meta_periodo || 0),
          Math.round(f.venda_digital || 0),
          Number((f.atingimento || 0).toFixed(1)),
          Math.round(f.gap || 0),
          Number((f.desvio || 0).toFixed(1)),
          Math.round(f.projecao || 0),
          Math.round(f.venda_total || 0),
          Math.round(f.venda_fisica || 0),
          Number((f.share_digital || 0).toFixed(1)),
          Number((f.share_digital_ly || 0).toFixed(1)),
          Number((f.diff_pp_ly || 0).toFixed(1)),
          Number((f.share_diretoria || 0).toFixed(2)),
          getStatusText(f.atingimento)
        ]);
      }});
      exportSheets({{ 'Filiais': rows }}, 'filiais_diretoria_c', format);
    }}

    function exportCategoriasFull(format = 'excel') {{
      const list = window.currentCategoriasList || [];
      const dirTot = getPeriodMetrics(DASH_DATA.total || {{}});
      const dirTotDigital = dirTot.venda_digital || 1.0;

      const catHeaders = [
        'Categoria / Grupo', 'Meta Mês (R$)', 'Meta Período (R$)', 'Venda Digital (R$)',
        'Atingimento %', 'GAP R$', 'Desvio %', 'Projeção Mês (R$)', 'Venda Total (R$)', 'Venda Física (R$)',
        'Share Atual %', 'Share LY %', 'Evol. P.P.', '% Share Diretoria', 'Total Linhas', 'Status'
      ];
      const catRows = [catHeaders];

      const linHeaders = [
        'Categoria / Grupo', 'Linha de Produto', 'Meta Mês (R$)', 'Meta Período (R$)', 'Venda Digital (R$)',
        'Atingimento %', 'GAP R$', 'Desvio %', 'Projeção Mês (R$)', 'Venda Total (R$)', 'Venda Física (R$)',
        'Share Atual %', 'Share LY %', 'Evol. P.P.', '% Share Categoria', '% Share Diretoria', 'Status'
      ];
      const linRows = [linHeaders];

      list.forEach(g => {{
        catRows.push([
          g.grupo,
          Math.round(g.meta_mes || 0),
          Math.round(g.meta_periodo || 0),
          Math.round(g.venda_digital || 0),
          Number((g.atingimento || 0).toFixed(1)),
          Math.round(g.gap || 0),
          Number((g.desvio || 0).toFixed(1)),
          Math.round(g.projecao || 0),
          Math.round(g.venda_total || 0),
          Math.round(g.venda_fisica || 0),
          Number((g.share_digital || 0).toFixed(1)),
          Number((g.share_digital_ly || 0).toFixed(1)),
          Number((g.diff_pp_ly || 0).toFixed(1)),
          Number((g.share_diretoria || 0).toFixed(2)),
          g.total_linhas || (g.linhas || []).length,
          getStatusText(g.atingimento)
        ]);

        (g.linhas || []).forEach(l => {{
          const lm = getPeriodMetrics(l);
          const share_grp = g.venda_digital > 0 ? ((lm.venda_digital / g.venda_digital) * 100) : 0.0;
          const share_dir = dirTotDigital > 0 ? ((lm.venda_digital / dirTotDigital) * 100) : 0.0;
          linRows.push([
            g.grupo,
            l.linha,
            Math.round(l.meta_mes || 0),
            Math.round(lm.meta_periodo || 0),
            Math.round(lm.venda_digital || 0),
            Number((lm.atingimento || 0).toFixed(1)),
            Math.round(lm.gap || 0),
            Number((lm.desvio || 0).toFixed(1)),
            Math.round(lm.projecao || 0),
            Math.round(lm.venda_total || 0),
            Math.round(lm.venda_fisica || 0),
            Number((lm.share_digital || 0).toFixed(1)),
            Number((lm.share_digital_ly || 0).toFixed(1)),
            Number((lm.diff_pp_ly || 0).toFixed(1)),
            Number(share_grp.toFixed(1)),
            Number(share_dir.toFixed(2)),
            getStatusText(lm.atingimento)
          ]);
        }});
      }});

      const sheets = {{ 'Categorias': catRows }};
      if (format === 'excel') {{
        sheets['Linhas por Categoria'] = linRows;
      }}
      exportSheets(sheets, 'categorias_diretoria_c', format);
    }}

    function exportLinhasFull(format = 'excel') {{
      const list = window.currentLinhasList || [];
      const headers = [
        'Ranking', 'Linha de Produto', 'Categoria / Grupo', 'Meta Mês (R$)', 'Meta Período (R$)',
        'Venda Digital (R$)', 'Atingimento %', 'GAP R$', 'Desvio %', 'Projeção Mês (R$)',
        'Venda Total (R$)', 'Venda Física (R$)', 'Share Atual %', 'Share LY %', 'Evol. P.P.',
        '% Share Diretoria', 'Status'
      ];
      const rows = [headers];
      list.forEach((l, idx) => {{
        rows.push([
          ...
          l.grupo,
          Math.round(l.meta_mes || 0),
          Math.round(l.meta_periodo || 0),
          Math.round(l.venda_digital || 0),
          Number((l.atingimento || 0).toFixed(1)),
          Math.round(l.gap || 0),
          Number((l.desvio || 0).toFixed(1)),
          Math.round(l.projecao || 0),
          Math.round(l.venda_total || 0),
          Math.round(l.venda_fisica || 0),
          Number((l.share_digital || 0).toFixed(1)),
          Number((l.share_digital_ly || 0).toFixed(1)),
          Number((l.diff_pp_ly || 0).toFixed(1)),
          Number((l.share_diretoria || 0).toFixed(2)),
          getStatusText(l.atingimento)
        ]);
      }});
      exportSheets({{ 'Linhas de Produtos': rows }}, 'linhas_produtos_diretoria_c', format);
    }}

    // Exportação genérica de tabelas do DOM (para compatibilidade)
    function exportTableToExcel(tableId, filename) {{
      const table = document.getElementById(tableId);
      if (!table) return;
      if (tableId === 'tableDistritaisOverview') return exportDistritaisOverview('excel');
      if (tableId === 'tableDistritaisFull') return exportDistritaisFull('excel');
      if (tableId === 'tableCoordenadoresFull') return exportCoordenadoresFull('excel');
      if (tableId === 'tableFiliaisFull') return exportFiliaisFull('excel');
      if (tableId === 'tableCategoriasFull') return exportCategoriasFull('excel');
      if (tableId === 'tableLinhasFull') return exportLinhasFull('excel');

      if (typeof XLSX !== 'undefined' && XLSX.utils) {{
        const wb = XLSX.utils.table_to_book(table, {{ sheet: "Dados" }});
        XLSX.writeFile(wb, filename.replace(/\\.csv$/i, '') + '.xlsx');
      }} else {{
        exportTableToCSV(tableId, filename.replace(/\\.xlsx$/i, '') + '.csv');
      }}
    }}

    function exportTableToCSV(tableId, filename) {{
      if (tableId === 'tableDistritaisOverview') return exportDistritaisOverview('csv');
      if (tableId === 'tableDistritaisFull') return exportDistritaisFull('csv');
      if (tableId === 'tableCoordenadoresFull') return exportCoordenadoresFull('csv');
      if (tableId === 'tableFiliaisFull') return exportFiliaisFull('csv');
      if (tableId === 'tableCategoriasFull') return exportCategoriasFull('csv');
      if (tableId === 'tableLinhasFull') return exportLinhasFull('csv');
      ...
      for (let i = 0; i < rows.length; i++) {{
        if (rows[i].classList.contains('accordion-child-row')) continue;
        const row = [];
      ...
      triggerDownload(csvFile, filename.endsWith('.csv') ? filename : filename + '.csv');
    }}
